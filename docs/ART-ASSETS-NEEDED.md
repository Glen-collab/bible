...
> fig leaves, and `saul` has the light beam built in).
>
...
> `eden`, `road`, `field`, and one mountain that serves both
> `sinai` (Moses) and `mount` (Jesus). Save each file with the **exact
...
| **One short word, no underscores** | `sea.png`, `tablets.png` | `red_sea.png`, `moses_10_command.png` |
...
| `sea`, `Split_red_sea` | `sea.png` **+** `split.png` | ✅ **Both are in.** You drew the sea closed *and* the sea parted, so they're now two backdrops: `place("sea")` shows the water, `place("split")` opens it. |
| `Moses_10_command` | `tablets.png` | ✅ **In.** It's Moses *holding* the tablets, so it's named for the man, not the stones — `place("tablets")` in the Red Sea workshop. |
| `harp` | `harp.png` | ✅ **In.** `place("harp")` in the David & Goliath workshop. |
...
| `eden`, `mount` | `eden.png`, `mount.png` | **new full-stage backdrops** — see "Backgrounds" |
...
`moses`, `tablets`, `crowd` have art; both Red Sea backdrops and the
`sinai` backdrop are wired. `staff` and `water` were **intentionally dropped** —
Moses holds his own staff, and the `split` backdrop is the water.

### 🌾 Case: The Barley Field *(Ruth)* — done ✅
`wheat` (the barley), `ox`, `well`, `house`, `sheep`, plus the `field` backdrop.

### ⚡ Case: The Damascus Road *(Paul)* — done ✅
Real art in: `saul` (Saul struck down, the light beam built in), `jesus`
(the risen Jesus appearing), `road` (the backdrop), plus `horse`, `donkey`,
`wheat`. Still emoji, all optional: `light` ⚡ (the beam is already in `saul`),
`palm` 🌴, `house` 🏠.
...
*(the den + lions use `den.png` and King Darius is `king.png` — both done ✅)*
...
`appletree` (the special tree, fruit + serpent already in its branches),
`cherub` (the angel with the flaming sword who guards the way — Gen 3:24), and the
story poses `peace` (at peace with the animals), `reach`,
`taste`, `bite`, `share`, and
`leave` (the sorrowful walk out). Backdrop `eden.png` ✅ is wired
(`place("eden")`).

| Filename | Now shows | Note |
|----------|-----------|------|
| `tree.png` | 🌳 | plain garden trees *(optional — `appletree` already covers the special one)* |
...
- `ark.png` — the ark *(command: `place("ark")`)*
- `tomb.png` — the tomb *(command: `place("tomb")`)*
- `sea.png` — the sea, still closed *(command: `place("sea")`)*
- `split.png` — the sea parted, dry path through *(command: `place("split")`)*
- `desert.png` — dry valley / desert, reusable *(command: `place("desert")`; auto-set in David's scene)*
- `eden.png` — the lush garden *(command: `place("eden")`; auto-set in Eden)*
- `road.png` — the winding road through the hills *(command: `place("road")`; auto-set in Paul's scene)*
- `field.png` — Ruth's golden field at harvest *(command: `place("field")`; auto-set in Ruth's scene)*
- `mount.png` — a mountaintop over a lake, **shared by two scenes**: `place("sinai")` (Moses / commandments) and `place("mount")` (Jesus's sermon; auto-set there)
...
> `place("split")` — and the water opens. The miracle happens by their own line
...
`assets/scenes/mount.png` and it overrides automatically — but it's optional.

> Done ✅: `eden` (`eden.png`), Paul's road (`road.png`), and
...
`crowd.png` · `feast.png` · `den.png` ·
`teaching.png` · `sermon.png` · `healing.png` ·
`fishbread.png` · `loaves.png`

---

## 4️⃣ Sprites already done ✅ (don't redo these)

`adam` · `bite` · `share` · `peace` ·
`leave` · `angel` · `armies` · `fawn` · `baby` · `boulder` ·
`butterfly` · `camel` · `cherub` · `chariot` · `cow` · `david` · `harp` · `deer` ·
`donkey` · `dove` · `elephant` · `eve` · `taste` ·
`reach` · `female` · `flamingo` · `appletree` · `giraffe` ·
`goliath` · `gorilla` · `hippo` · `horse` · `house` · `jesus` *(risen)* · `joseph` ·
`kangaroo` · `king` · `lion` · `man` · `mary` · `monkey` · `moses` · `tablets` ·
`mouse` · `noah` · `ostrich` · `owl` · `ox` · `palm` · `panda` · `rainbow` · `rhino` ·
`saul` · `serpent` · `sheep` · `snail` · `squirrel` · `tiger` · `toucan` ·
`tree` *(olive)* · `well` · `wheat` *(barley)* · `wisemen` · `zebra`

> **Two figures have a second pose:** `david` (with the sling) and `harp` (the
> shepherd boy on his rock); `moses` (staff raised at the sea) and `tablets`
...
The only in-scene emoji is the Damascus `light` ⚡ (optional — `saul` already
has the beam). Drop an `assets/sprites/light.png` someday if you want it; otherwise
you're finished.

Backdrops, for reference — all wired and live:
`manger` · `ark` · `tomb` · `sea` / `split` · `desert` · `eden`
(`eden`) · `road` · `field` · `sinai` = `mount` (shared).
